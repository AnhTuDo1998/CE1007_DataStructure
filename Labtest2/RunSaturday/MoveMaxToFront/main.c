...
int moveMaxToFront(ListNode **ptrHead)
{
    ...
    cur = *ptrHead;
    preMax = NULL; // at the end if preMax is still null then it should be max value in front
    max_item = (*ptrHead)->item;
    index = 0;
    //traversing the list
    while(cur!=NULL){
        if (cur->item>max_item){
            max_item = cur->item;
            preMax = pre;
        }
        pre = cur;
    ...
    //after traversing, if preMax == null, means return 0
    if (preMax==NULL)
        return 0;
    else//move max value in the front
    {
        cur = preMax->next;
