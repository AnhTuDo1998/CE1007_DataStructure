...
void RecursiveReverse(ListNode **ptrHead)
{
	...
	if(*ptrHead==NULL)
        return;
    if (rest == NULL)
        return; //1 element cannot reverse

    else{
        RecursiveReverse(&rest);
        first->next->next = first;//first-> next is the last node in reverse next.this last node point to first
        first->next = NULL;//remove the previous link
        *ptrHead = rest;
    }
}
