...
void createStackFromLinkedList(LinkedList *ll, Stack *s)
{
    ...
    int digit_from_list;
    if (ll == NULL || s == NULL)
        return; // no allocate memory in the first place
    if (!isEmptyStack(s))
        removeAllItemsFromStack(s);//remove all for non empty stack
    for (i=0;i<ll->size;i++){
        digit_from_list = findNode(ll,i)->item;
        push(s,digit_from_list);
    }
...
{
	/* add your code here */
	Stack t;//create and initialise another stack to do stuff
	int item;
	t.ll.head = NULL;
	t.ll.size = 0;
	if (isEmptyStack(s))
