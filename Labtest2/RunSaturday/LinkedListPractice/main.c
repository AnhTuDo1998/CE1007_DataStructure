...
int concatenate(ListNode **ptrHead1, ListNode *head2){
    ListNode *cur, *pre;
	// write your code here
	//if both list empty, return, if list 2 is empty, return list 1 no need do, if list 1 is empty, return list 2 (point to)
    //*ptrHead1 = NULL;
    //head2 = NULL;
    ...
    pre->next = head2;
    return 0;
    //if both are not empty, we must traverse all the way to the back of the first ll, then use next to connect to head of pointer 2 of ll2
}


int combineAlternating(ListNode **ptrHead, ListNode *head1, ListNode *head2){
// if head1, head2 both empty
    ListNode *temp;
    if (head1 == NULL && head2 == NULL)
        return -1;
    while (head1!=NULL||head2!=NULL){
        //if ptrHead is empty ( not allocateD
        if (*ptrHead == NULL)
        {
            if (head1!=NULL)
            {
                *ptrHead = malloc(sizeof(ListNode));//pointer is not a node, point to the allocation of a node
                (*ptrHead)->num = head1->num; //if you use ptrHead = temp and then temp = malloc then ptrHead point to nothing!!!!
                temp = *ptrHead;
                head1=head1->next;
                if(head2!=NULL)
                {
