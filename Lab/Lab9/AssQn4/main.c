...
{
	/* add your code here */
	int break_node_index;
	int i;
	int j = 0;
	int k = 0;
	if ((ll->size)%2==0)
        break_node_index = (ll->size)/2;
    else
        break_node_index = (ll->size)/2 + 1;
    for (i = 0; i<ll->size; i++){
        if (i<break_node_index){
            insertNode(resultFrontList,j,findNode(ll,i)->item);
            j++;

        }
        else{
            insertNode(resultBackList,k,findNode(ll,i)->item);
            k++;
        }
    }
}
