...
{
    /* add your code here */
    //create and initialize a queue according to the qns requirement:
    Queue q;
    q.head = NULL;
    q.tail = NULL;

    BSTNode *temp; // declare a temporary pointer

    temp = root; //assign to root node
    if (temp==NULL) //if empty tree
        return;

    else{ //if tree is not empty (this case will be mutually exclusive!
        enqueue(&q.head,&q.tail, temp); //put first element into the Q

        while (!isEmpty(q.head)) // while the queue is not empty
        {
            temp = dequeue(&q.head,&q.tail);
            printf("%d", temp->item);
            //add in left and right children (if not null of course
            if (temp->left!=NULL)
